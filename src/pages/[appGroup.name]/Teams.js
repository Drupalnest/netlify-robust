...
const TeamList = ({ responseData }) => {
  ...
  const fetchTeamsAndApps = (appGroupName) => {
    dispatch(fetchTeams());
    dispatch(fetchApps(appGroupName)); // Assuming appGroupName is the parameter you want to pass
  };
  ...
  useEffect(() => {
    ...
    // Set up interval for automatic refresh (every 60 seconds)
    const interval = setInterval(() => {
      fetchTeamsAndApps();
    }, 60000);
