...
exports.handler = async (event, context) => {
  try {
    ...
    const keyFilePath = path.resolve(process.cwd(), 'token', 'node', 'getTokenWithServiceAccount', 'inspiring-bonus-405815-b81c6343d863.json');

    console.log('Script Path:', scriptPath);
    console.log('Key File Path:', keyFilePath);

    // Use the NODE_PATH environment variable to set the paths for ESM
    process.env.NODE_PATH = path.join(process.cwd(), 'token', 'node', 'getTokenWithServiceAccount');
